...
// ✅ Safe JSON Decode Helper to prevent "Cannot access offset of type string on string"
function safeJsonDecode($response) {
    $decoded = json_decode($response, true);
    if (!is_array($decoded)) {
        return ['error' => 'Invalid JSON response from LHDN', 'raw' => $response];
    }
    return $decoded;
}
...
function extractLhdnError($rec) {
    if (in_array($rec['lhdn_status'] ?? '', ['Invalid', 'Error']) && !empty($rec['lhdn_response'])) {
        $r = safeJsonDecode($rec['lhdn_response']);
    ...
    try {
        ...
        if ($_GET['ajax_action'] === 'process_next') {
            ...
            if ($indRec) {
                ...
                // ✅ SAFE JSON DECODE
                $submitResponse = safeJsonDecode($submitResult['response']);
                ...
                if ($lhdnStatus === 'Submitted' && $uuid) {
                    ...
                    $detailsResponse = safeJsonDecode($statusResult['response']);
