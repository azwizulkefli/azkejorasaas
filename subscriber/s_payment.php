...
// ============================================================
// 1. FETCH ALL PAYMENT RECORDS from subscriptions_billing
// ============================================================
$stmt = $pdo->prepare("
    SELECT sb.*, s.status as sub_status
    FROM subscriptions_billing sb
    LEFT JOIN subscriptions s ON sb.subscription_id = s.id
    WHERE sb.user_id = ?
    ORDER BY sb.payment_date DESC NULLS LAST, sb.created_at DESC
");
$stmt->execute([$uid]);
$payments = $stmt->fetchAll(PDO::FETCH_ASSOC);

// ============================================================
// 2. SUMMARY STATS & DATA ENRICHMENT
// ============================================================
$totalPaid     = 0.0;
$countPaid     = 0;
$lastPaymentAt = null;

$payTypeLabel = function($method, $gateway) {
    $method = strtolower((string)$method);
    $gateway = strtolower((string)$gateway);
    if (strpos($gateway, 'stripe') !== false) return '💳 Card (Stripe)';
    if (strpos($gateway, 'fpx') !== false) return '🏦 FPX Online Banking';
    if (strpos($gateway, 'toyyibpay') !== false || strpos($method, 'duitnow') !== false) return '📱 ToyyibPay / DuitNow';
    if (strpos($method, 'tng') !== false) return '📱 Touch \'n Go eWallet';
    if (strpos($method, 'manual') !== false || strpos($gateway, 'manual') !== false) return '💵 Manual Transfer';
    return ucfirst(str_replace('_', ' ', $method ?: $gateway)) ?: '—';
};

$statusBadge = fn($st) => [
    'paid'                 => ['bg:#d1fae5','color:#059669','Paid'],
    'pending_verification' => ['bg:#fef3c7','color:#d97706','Pending Verification'],
    ...
    'failed'               => ['bg:#ffe4e6','color:#e11d48','Failed'],
][$st] ?? ['bg:#f1f5f9','color:#94a3b8', ucfirst((string)$st) ?: '—'];

// Enrich payment rows with decoded metadata for easy JS/HTML use
foreach ($payments as &$p) {
    $meta = json_decode($p['metadata'] ?? '{}', true);
    
    // Extract bank and ref from metadata, with fallbacks
    $p['bank_display'] = $meta['bank_name'] ?? ($p['payment_gateway'] === 'stripe' ? 'Stripe' : ($p['payment_gateway'] === 'toyyibpay' ? 'ToyyibPay' : '—'));
    $p['ref_display']  = $meta['ref_no'] ?? $p['transaction_id'] ?? '—';
    $p['method_label'] = $payTypeLabel($p['payment_method'], $p['payment_gateway']);
    
    // Only count 'paid' transactions towards lifetime stats
    if ($p['payment_status'] === 'paid') {
        $totalPaid += (float)($p['amount'] ?? 0);
        $countPaid++;
        if (!$lastPaymentAt) {
            $lastPaymentAt = $p['payment_date'];
        }
    }
}
...
.btn-view{display:inline-flex;align-items:center;gap:6px;background:#eef1ff;color:#4644cf;border-radius:10px;padding:8px 14px;font-size:12px;font-weight:700;transition:.15s;border:none;cursor:pointer}
.btn-view:hover{background:#c6ceff;color:#fff}
.btn-ghost{background:#fff;border:1px solid var(--line);color:var(--ink);border-radius:12px;padding:11px 18px;font-size:13px;font-weight:700;display:inline-flex;align-items:center;gap:8px;cursor:pointer;text-decoration:none}
...
@media print {
  ...
  #printArea { position: absolute !important; left: 0; top: 0; width: 100%; padding: 0 !important; margin: 0 !important; background: #fff !important; }
    ...
    <div class="table-wrap">
      <?php if (empty($payments)): ?>
        <div class="empty-state">
          ...
          <p>Your payments from Manual Transfer, DuitNow, and Touch 'n Go will appear here after your first successful charge.</p>
        </div>
      <?php else: ?>
        <table class="pay-table">
          ...
          <tbody id="paymentsBody">
            <?php foreach ($payments as $p):
              $sb = $statusBadge($p['payment_status'] ?? 'none');
            ?>
            <tr data-search="<?= strtolower(($p['receipt_no'] ?? '') . ' ' . $p['ref_display'] . ' ' . $p['bank_display'] . ' ' . ($p['payment_method'] ?? '')) ?>"
              ...
              <td><?= htmlspecialchars($p['method_label']) ?></td>
              <td>
                <span class="mono"><?= htmlspecialchars($p['ref_display']) ?></span>
                <?php if (!empty($p['bank_display']) && $p['bank_display'] !== '—'): ?>
                  <div style="font-size:11px;color:var(--faint);margin-top:2px"><?= htmlspecialchars($p['bank_display']) ?></div>
                <?php endif; ?>
              </td>
              ...
              <td style="text-align:right">
                <!-- Note: json_encode outputs valid JS object syntax, safe inside single-quoted onclick -->
                <button class="btn-view" onclick='openReceipt(<?= json_encode($p) ?>)'>
                  📄 View Receipt
                </button>
              </td>
            </tr>
...
<div class="modal" id="receiptModal" onclick="if(event.target===this)closeReceipt()">
  <div class="modal-card">
    <div class="modal-head">
      ...
      <div class="actions">
        <button class="btn-ghost" onclick="printReceipt()" style="padding:8px 14px">🖨 Print / Save PDF</button>
      ...
      <div id="printArea" class="receipt">
        <div class="receipt-head">
          <div class="company">
            ...
            <p>E-Invoice & Facility Booking Platform<br>Kuala Lumpur, Malaysia<br>support@azkejora.io · +60 3-0000 0000</p>
          </div>
          <div class="tag">
        ...
        <div class="receipt-section">
          ...
          <div class="pay-breakdown">
            <div class="row"><span>Subscription Plan</span><b id="r-plan">—</b></div>
            <div class="row"><span>Billing Cycle</span><b>90 days (3 months)</b></div>
            <div class="row"><span>Period Start</span><b id="r-period-start">—</b></div>
            <div class="row"><span>Period End</span><b id="r-period-end">—</b></div>
            <div class="row"><span>Subtotal</span><span id="r-subtotal">RM 0.00</span></div>
            <div class="row"><span>SST (8%)</span><span id="r-sst">RM 0.00</span></div>
            <div class="row total"><span>TOTAL PAID</span><span id="r-total">RM 0.00</span></div>
          </div>
        </div>

        <div class="receipt-footer">
          <p><strong>Thank you for your payment.</strong><br>
          This receipt was generated automatically by the AZ Kejora SaaS payment gateway.<br>
          For queries, contact <strong>support@azkejora.io</strong>.</p>
...
function openReceipt(p) {
  ...
  if (p.billing_period_end) {
    const end = new Date(p.billing_period_end);
    const start = new Date(p.billing_period_start || p.billing_period_end);
  ...
  document.getElementById('r-method').textContent = p.method_label || p.payment_method || '—';
  document.getElementById('r-bank').textContent = p.bank_display || '—';
  document.getElementById('r-ref').textContent = p.ref_display || '—';
